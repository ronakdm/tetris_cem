...
for it in range(iters):
    ...
    def worker(p):
        ...
        #players_lines_cleared[p] = np.round(np.mean(all_lines_cleared), decimals=2)
        #print(players_lines_cleared[p], end=" ", flush=True)
    ...
    players_lines_cleared = np.array(Parallel(n_jobs=-2)(
        delayed(worker)(p) for p in range(num_players)
    ))

    # Option 2: Carry over elite_rewards and do not.
    ...
    # if prev_best:
    #     elite_weights = np.concat([prev_best, elite_weights])
    # prev_best = elite_weights[-num_best:]

    # TODO: Uniform sampling as well. ~ (if time)

    # Recompute Gaussian distribution
    ...
    # dist_from_goal = goal - elite_lines_cleared
    # if dist_from_goal <= 0:
    #     num_players = end_num_players
    #     num_episodes = end_num_episodes
    # num_players = end_num_players + int((dist_from_goal / goal * (start_num_players - end_num_players)))
    # num_episodes = start_num_episodes + int(dist_from_goal / goal * (end_num_episodes - start_num_episodes
    ...
    # print("Avg time to compute feature vector:", 0)  # TODO:
    # print("Avg time to compute an action:", 0)  # TODO:
    # print("Avg time to run episode:", 0)  # TODO:
    # print("Avg time to compute fitness function for player:", 0)  # TODO:
    ...
    # print("Avg lines cleared by the elite set:", 0)  # TODO:
